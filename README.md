...
- 发布步骤：先执行一次同步以补齐 `isAI`、`era`、`descriptionLong`，再验证首页「经典 AI 项目 / 最新 AI 项目」两个栏目都能正常展示。
- 验证要点：确认非 AI 项目不会出现在任一栏目；确认经典栏按影响力排序、最新栏按更新时间排序；确认长描述在桌面与移动端都能稳定截断显示。
- 回滚策略：若出现严重问题，可暂时下线 AI 分栏区域（前端禁用分栏展示），并回退到原有列表查询路径。
